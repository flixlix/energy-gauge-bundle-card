...
import { formatNumber, FrontendLocaleData } from 'custom-card-helpers';
import { blankBeforePercent } from '../utils/blank-before-percent';
import { afterNextRender } from '../utils/render-status';
import { getValueInPercentage, normalize } from '../utils/calculate';
    ...
    return svg`
      ...
      <svg class="text">
        <text class="value-text">
          ${this._segment_label ? this._segment_label : this.valueText || formatNumber(this.value, this.locale, this.formatOptions)}${
      this._segment_label ? '' : this.label === '%' ? blankBeforePercent(this.locale) + '%' : ` ${this.label}`
    }
        </text>
      </svg>`;
  }
